...
namespace	ft
{
	...
	{
		if (head == this->_null_node || \
				(!this->comp_data(head->data, key) && \
				 !this->comp_data(key, head->data)))
			return (head);
		if (this->comp_data(key, head->data))
	...
	*/
	...
	template <
		...
		typename Alloc,
		bool multivalues
	> int	binary_search_tree<T, KeyOfValue, Compare, Alloc, multivalues>::tree_insert
		(node_ptr new_node)
	{
		node_ptr	tmp;
		node_ptr	parent;
